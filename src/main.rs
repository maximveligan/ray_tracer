...
fn color_mul_const(color: &Rgb<f64>, constant: f64) -> Rgb<f64> {
    Rgb {
        data: [
            ((color.data[0] * constant) / 255.0).min(255.0).max(0.0),
            ((color.data[1] * constant) / 255.0).min(255.0).max(0.0),
            ((color.data[2] * constant) / 255.0).min(255.0).max(0.0),
        ],
    }
}
...
fn color_mul_color(color: &Rgb<f64>, color2: &Rgb<f64>) -> Rgb<f64> {
    Rgb {
        data: [
            ((color.data[0] * color2.data[0]) / 255.0)
                .min(255.0)
                .max(0.0),
            ((color.data[1] * color2.data[1]) / 255.0)
                .min(255.0)
                .max(0.0),
            ((color.data[2] * color2.data[1]) / 255.0)
                .min(255.0)
                .max(0.0),
        ],
    }
...
impl Ray {
    fn create_prime(x: u32, y: u32, scene: &Scene) -> Ray {
        ...
        let canvas_x =
            (((x as f64 + HALF_PIXEL) / (scene.width as f64 / 2.0)) - 1.0) * aspect_ratio;
                    ...
                    if let Some(d) = object.intersection(&ray) {
                        match self.light {
                            Light::DirLight(ref d_l) => {
                                let light_p = (object.get_albedo() / std::f64::consts::PI)
                                    * d_l.intensity
                                    * object
                                        .normal_vec(&(ray.origin + (ray.direction * d)))
                                        .dot(-d_l.direction.normalize())
                                        .max(0.0);
                                image.put_pixel(
                                    x,
                                    y,
                                    rgbf64_to_rgb8(&color_mul_const(
                                        &color_mul_color(&object.get_color(), &d_l.color),
                                        light_p,
                                    ))
                                    .to_rgba(),
                                );
...
fn rgbf64_to_rgb8(color: &Rgb<f64>) -> Rgb<u8> {
    Rgb {
        data: [
            color.data[0].round() as u8,
            color.data[1].round() as u8,
            color.data[2].round() as u8,
        ],
    }
}
